...
public class ConsultaSaldo implements HelloController{
    ...
    private void contenido(GridPane gridPane){
        ...
        Label titulo = new Label("Consulta De Saldo");
        ...
        Label id = new Label("Ingresa la id del cliente a consultar saldo");
            ...
            public void handle(ActionEvent e) {
                ...
                for(int i = 0; i<listaC.size(); i++){
                    if(id == listaC.get(i).getId()){
                        showAlert(Alert.AlertType.CONFIRMATION, gridPane.getScene().getWindow(), "Saldo", "El saldo del usuario "
                        + listaC.get(i).getId() + " es $" +listaC.get(i).getSaldo());
                        x = 1;
                    }
                }
                if (x == 0){
